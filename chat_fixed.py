...
def process_message(sentence: str, image_path: Optional[str] = None) -> str:
    sentence = (sentence or "").strip()
    # Nếu có ảnh, quét OCR và kết hợp với sentence
    ocr_text = ""
    if image_path and os.path.exists(image_path):
        try:
            print(f"[OCR] Đang quét ảnh: {image_path}")
            results = OCR_READER.readtext(str(image_path), detail=0, paragraph=True)
            ocr_text = "\n".join(results).strip()
            if ocr_text:
                sentence = f"{sentence}\n\n[Văn bản từ ảnh]:\n{ocr_text}" if sentence else f"[Văn bản từ ảnh]:\n{ocr_text}"
            print(f"[OCR] Thành công – {len(ocr_text)} ký tự")
        except Exception as e:
            print(f"[OCR] Lỗi: {e}")
            sentence += "\n[OCR] Không đọc được văn bản trong ảnh."
    if not sentence:
